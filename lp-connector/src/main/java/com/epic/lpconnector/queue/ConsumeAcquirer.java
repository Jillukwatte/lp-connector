...
@Component
public class ConsumeAcquirer {
	ExecutorService  executor = Executors.newCachedThreadPool();
	boolean isretarting =false;
	Logger LOGGER = LoggerFactory.getLogger(ConsumeAcquirer.class);
	 
	 
//	@RabbitListener(queues = "#{@ceft_queue_acquirer}" ,concurrency = "#{@concurrent_consumers}" )   //  #{@somePropertyValue} 
	@RabbitListener(queues = "ceft_queue_acquirer" ,concurrency = "#{@concurrent_consumers}" )   //  #{@somePropertyValue}
	...
	public CeftResponse TierARabbitListener(Session s) {
		...
		try {
			Marker uuid = MarkerFactory.getMarker(s.getCeftRequest().getUuid());
			LOGGER.info(uuid, "Consumed by TierARabbitListener- {}" ,s.getCeftRequest()); 
		...
		if(flag) {
			...
			ceftResponse.setDescription("Succesfully accepted.");  
			...
			ceftResponse.setDescription("RejectedExecution Exception occured at TierARabbitListener");
		} catch (Exception e) {
			e.printStackTrace();
			ceftResponse.setResponsecode("ES09");
			ceftResponse.setDescription("Exception occured at TierARabbitListener");
		} 
